...
class TempHumid
{
  public:
    ...
    void readPIR();
    float readTemperatureC(void (*f1)(byte), RF24& radio);// can radio be const?
    float readHumidity(float tempC, void (*f1)(byte), RF24& radio);
  private:
    int _dataPin;
    int _clockPin;
    ...
    float readTemperatureRaw(void (*f1)(byte), RF24& radio);
    void waitForResultSHT(int _dataPin, void (*f1)(byte), RF24& radio);
};
