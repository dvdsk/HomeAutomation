...
pub fn handle_cmd(cmd: Command, mods: &mut Modifications, sys: &mut System) -> Option<State> {
    ...
    match cmd {
        ...
        Command::LampsToggle => {
            sys.lights.toggle().unwrap();
            mods.lighting = true
        }
        Command::LampsDim => {
            sys.lights.set_all_ct(50, 500).unwrap();
            mods.lighting = true
        }
        Command::LampsDimmest => {
            sys.lights.set_all_ct(1, 500).unwrap();
            mods.lighting = true
        }
        Command::LampsEvening => {
            sys.lights.set_all_ct(254, 320).unwrap();
            mods.lighting = true
        }
        Command::LampsNight => {
            sys.lights.set_all_ct(220, 500).unwrap();
            mods.lighting = true
        }
        Command::LampsDay => {
            sys.lights.set_all_ct(254, 240).unwrap();
            mods.lighting = true
        }
        ...
        Command::MpdPause => {
            mpd_control::toggle_playback(&mut sys.mpd).unwrap();
            mods.mpd = true
        }
        Command::MpdDecreaseVolume => {
            mpd_control::decrease_volume(&mut sys.mpd).unwrap();
            mods.mpd = true
        }
