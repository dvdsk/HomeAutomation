...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Error::CouldNotDownloadSong(e) => write!(
                f,
                "Could not download song, cause: {}",
                String::from_utf8(e.stderr.clone()).unwrap()
            ),
            Error::CouldNotUpdateMpd(e) => write!(f, "Could not update mpd, cause: {}", e),
            Error::CouldNotCreateTempDir(e) => {
                write!(f, "Could not create temp directory, cause: {}", e)
            }
            Error::UnexpectedYoutubeDlStdOut(s) => write!(
                f,
                "Could not understand output of youtube-dl, output was: {}",
                s
            ),
            Error::UnsupportedSource(s) => write!(f, "Could not find something to download: {}", s),
            Error::CanNoLongerUpdateMeta => write!(
                f,
                "Can no longer update metadata, was already done some time ago"
            ),
            Error::CanNotSwapArtistWithEmptyTitle => write!(
                f,
                "Can not swap artist and title as the current artist \
                is empty and the title can not be empty. Not writing metadata"
            ),
            Error::IDWasDeleted => write!(f, "Internal error, file not in database"),
        }
    }
}
...
fn download_file(output_arg: &String, url: &str) -> std::io::Result<std::process::Output> {
    let full_path = Path::new(YOUTUBE_DL_LOC);
...
async fn download_song(url: &str, id: u64) -> Result<(), Error> {
    ...
    if !temp_dir.exists() {
        fs::create_dir(&temp_dir).map_err(|e| Error::CouldNotCreateTempDir(e))?;
    }
    ...
    if !output.status.success() || output.stderr.starts_with(b"ERROR:") {
        ...
        let since_last_updated = fs::metadata(YOUTUBE_DL_LOC)
            .unwrap()
            .created()
            .unwrap()
            .elapsed()
...
async fn handle_job(job: Job, token: String, db: sled::Tree) -> Result<(), Error> {
    let status = match download_song(&job.url, job.id).await {
        Ok(_) => {
            aquire_db_mutex(&db, job.id).await;
            let db_status = db
                .get(&status_key(job.id))
                .unwrap()
            ...
            let (new_db_status, job_status) = match db_status {
                ...
                Status::Downloaded(_) => panic!("file should not yet be download!"),
                Status::MetaWritten((_, _)) => panic!("file should not yet be download!"),
            };
...
fn song_downloader(url_rx: crossbeam_channel::Receiver<Job>, token: String, db: sled::Tree) {
...
impl YoutubeDownloader {
    pub async fn init(
        ...
        let downloader_thread =
            thread::spawn(move || song_downloader(url_rx, local_token, youtube_db));
    ...
    pub async fn add_song_to_queue(
        ...
        let mut batch = Batch::default();
        batch.insert(&status_key(id), bincode::serialize(&db_entry).unwrap());
        batch.insert(&db_mutex_key(id), bincode::serialize(&false).unwrap());
        db.apply_batch(batch).unwrap();
        ...
        feedback
            .send(JobStatus::Queued(meta_guess, id), &self.token)
            .await;
        dbg!();
        Ok(())
    }

    pub async fn meta_ok(&self, id: u64) -> Result<(), Error> {
        let db = self.db.open_tree("youtube_dl").unwrap();
        ...
        let mut batch = Batch::default();
        batch.insert(&status_key(id), bincode::serialize(&new_status).unwrap());
        batch.insert(
            //unlock file lock
            &db_mutex_key(id),
            bincode::serialize(&false).unwrap(),
    ...
    pub async fn swap_meta(&self, id: u64) -> Result<(), Error> {
        let db = self.db.open_tree("youtube_dl").unwrap();
    ...
    pub async fn no_meta(&self, id: u64) -> Result<(), Error> {
        let db = self.db.open_tree("youtube_dl").unwrap();
