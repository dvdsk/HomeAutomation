...
pub fn handle_cmd(cmd: Command, state: State, mods: &mut Vec<Modification>, sys: &mut System) -> State {
    ...
    match cmd {
      ...
      Command::LampsToggle => {sys.lights.toggle().unwrap(); mods.push(Modification::Lighting)},
			Command::LampsDim => {sys.lights.set_all_to(50,500).unwrap(); mods.push(Modification::Lighting)},
			Command::LampsDimmest => {sys.lights.set_all_to(1,500).unwrap(); mods.push(Modification::Lighting)},
			Command::LampsEvening => {sys.lights.set_all_to(254,320).unwrap(); mods.push(Modification::Lighting)},
			Command::LampsNight => {sys.lights.set_all_to(220,500).unwrap(); mods.push(Modification::Lighting)},
			Command::LampsDay => {sys.lights.set_all_to(254,240).unwrap(); mods.push(Modification::Lighting)},
